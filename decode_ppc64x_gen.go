...
func main() {
	...
	// Translate/validate nibble LUTs (Muła), one 16-byte vector each.
	lutLo := f.Data("dppcLutLo", []byte{0x15, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x13, 0x1A, 0x1B, 0x1B, 0x1B, 0x1A})
	lutHi := f.Data("dppcLutHi", []byte{0x10, 0x10, 0x01, 0x02, 0x04, 0x08, 0x04, 0x08, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10})
	lutRoll := f.Data("dppcLutRoll", []byte{0, 16, 19, 4, 0xBF, 0xBF, 0xB9, 0xB9, 0, 0, 0, 0, 0, 0, 0, 0})
	...
	b := ppc64.NewFunc("decodeBlocks", sig, 0)
	b.LoadArg("dst_base", "R3").
		...
		Raw("MOVD $%s(SB), R6", c2f).Raw("LXVB16X (R6)(R0), VS50").     // V18 = 0x2f
		...
		Raw("VCMPEQUB V0, V18, V6"). // 0xFF where char == '/'
		Raw("VADDUBM V2, V6, V6").   // hi + eq2f
