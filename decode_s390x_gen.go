...
func main() {
	...
	lutLo := f.Data("ds390LutLo", []byte{0x15, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x13, 0x1A, 0x1B, 0x1B, 0x1B, 0x1A})
	lutHi := f.Data("ds390LutHi", []byte{0x10, 0x10, 0x01, 0x02, 0x04, 0x08, 0x04, 0x08, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10})
	lutRoll := f.Data("ds390LutRoll", []byte{0, 16, 19, 4, 0xBF, 0xBF, 0xB9, 0xB9, 0, 0, 0, 0, 0, 0, 0, 0})
	...
	c2f := f.Data("ds390C2f", splat(0x2f))
	...
	g := s390x.NewFunc("decodeBlocks", sig, 0)
	g.LoadArg("dst_base", "R1").
		...
		Raw("MOVD $%s(SB), R4", c2f).Raw("VL (R4), V24").
		...
		Raw("VCEQB V0, V24, V6").      // 0xFF where char == '/'
		Raw("VAB V2, V6, V6").         // hi + eq2f
