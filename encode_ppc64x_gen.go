...
func main() {
	...
	// ASCII offset-LUT, keyed by the range bucket. For StdEncoding (+/) the last two
	// non-zero entries 237,240 shift index 62->'+' and 63->'/'; for URL/RawURL (-_)
	// they become 239,32 (62->'-', 63->'_'). All other entries are alphabet-shared.
	...
	for _, vr := range []struct {
		...
		lut := f.Data("ppcLut"+vr.suffix, vr.lutB)
		b := ppc64.NewFunc("encodeBlocks"+vr.suffix, sig, 0)
